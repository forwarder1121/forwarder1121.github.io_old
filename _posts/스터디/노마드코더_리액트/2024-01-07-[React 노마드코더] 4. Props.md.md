...
이 porps 덕분에 우리는 재사용성이 높은 웹사이트 코드를 제작할 수 있는데, 어떻게 그게 가능한지 알아보도록 하자.
...
이렇게 부모 컴포넌트인 App 컴포넌트에서 Btn을 호출할 때 text를 인자로 주고 Btn은 그 text를 이용해 생성되면, 아까와 같이 불필요한 중복이 필요 없게 된다. 
...
> 사용자 정의 컴포넌트에 들어가는 속성은 HTML속성이 아니라 컴포넌트의 생성시 Props에 들어가는 데이터이기 때문

따라서 이미지 태그를 따로 생성하고, Props를 이용해서 src를 다시 적용해주어야 한다.
...
React는 부모 Component가 상태 변경을 겪을때, 모든 Component가 새롭게 계산된다(re-rendering)
...
위와 같이 String이 들어와야 할 자리에 bool타입의 값이 들어가면 코드는 문법적으로 에러는 없는데, 논리적으로 내가 원하는 결과가 나오지 않을 것.  
